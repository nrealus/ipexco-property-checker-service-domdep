    ...
    def _make_htn_add_loading(self, flights_index: int, last_unload_before_loads):
        ...
        for i, jig_type_name in flights.outgoing.items():
            ...
            if jig_type_name.startswith("jig"):
                concrete_jig = jig_type_name
                jig_type_name = self.pb_def.get_jig(jig_type_name).type
            else:
                concrete_jig = None
            ...
            if concrete_jig is not None:
                self.pb.task_network.add_constraint(up.Equals(self.loads_jig_vars[(flights.name, i)], self.jig_objects[concrete_jig]))
...
# vvv NOTE vvv !!!! Independent of planner !!!!
def check_plan_properties(
    ...
    props_unload_beluga: dict[tuple[str, int, int], str] = {}
    props_load_beluga: dict[tuple[str, int, int], str] = {}
    ...
    props_jig_to_production_line_before_flight: dict[tuple[str, str, int], str] = {}
    ...
    for (prop_id, prop_name_and_params) in properties.items():
        ...
        if prop_name == "unload_beluga":
            j, bi, ji = prop_params[0], int(prop_params[1]), int(prop_params[2])
            props_unload_beluga[(j, bi, ji)] = prop_id

        elif prop_name == "load_beluga":
            j, bi, ji = prop_params[0], int(prop_params[1]), int(prop_params[2])
            props_load_beluga[(j, bi, ji)] = prop_id
            ...
            j, pl, bi = prop_params[0], prop_params[1], int(prop_params[2]),
            props_jig_to_production_line_before_flight[(j, pl, bi)] = prop_id
    ...
    unloads_encountered: dict[int, int] = {}    # key: beluga index/number, value: number of unloads from beluga
    loads_encountered: dict[int, int] = {}      # key: beluga index/number, value: number of loads to beluga
    delivers_encountered: dict[str, int] = {}   # key: production line name, value: number of jigs delivered to it
    switches_to_next_flight_encountered: set[int] = set()   # values: flight index / number

    jigs_delivered_to_pl: dict[str, set[str]] = {}
    
    swaps_picks_and_puts: dict[tuple[str, str, str], int] = {}

    def beluga_index_from_name(b:str):
        return int(b.removeprefix("beluga"))-1

    for act in plan_def:

        if act.name == "unload_beluga":
            j = act.params['j']
            bi = beluga_index_from_name(act.params['b'])
            ji = unloads_encountered.setdefault(bi, 0)

            prop_id = props_unload_beluga.get((j, bi, ji), None)
            if prop_id is not None:
                props_satisfied[prop_id] = True

            unloads_encountered[bi] += 1

        elif act.name == "load_beluga":
            j = act.params['j']
            bi = beluga_index_from_name(act.params['b'])
            ji = loads_encountered.setdefault(bi, 0)

            prop_id = props_load_beluga.get((j, bi, ji), None)
            if prop_id is not None:
                props_satisfied[prop_id] = True

            prop_id = props_load_beluga.get((jig_types[j], bi, ji), None)
            if prop_id is not None:
                props_satisfied[prop_id] = True

            loads_encountered[bi] += 1
            ...
            switches_to_next_flight_encountered.add(len(switches_to_next_flight_encountered))
